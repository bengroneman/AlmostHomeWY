...
const StoryCard = class extends React.Component {
    ...
    render() {
        return (
            <div key={this.props.story.title} className="flex flex-col rounded-lg shadow-lg overflow-hidden">
                <div className="flex-shrink-0">
                </div>
                <div className="flex-1 bg-white p-6 flex flex-col justify-between">
                    <div className="flex-1">
                        <a href={this.props.story.slug} className="block mt-2">
                            <p className="text-xl font-semibold text-gray-900">{this.props.story.title}</p>
                            <p className="mt-3 text-base text-gray-500">{this.props.story.description}</p>
                        </a>
                    </div>
